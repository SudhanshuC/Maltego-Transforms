...
from common.entities import MyHaveibeenpwnedEntity
...
"""
@configure(
    label='To MyHaveibeenpwnedEntity [Hello World]',
    description='Returns a MyHaveibeenpwnedEntity entity with the phrase "Hello Word!"',
    uuids=[ 'haveibeenpwned.v2.MyHaveibeenpwnedEntityToPhrase_HelloWorld' ],
    inputs=[ ( 'Haveibeenpwned', Person ) ],
    remote=False,
    debug=True
)
def dotransform(request, response, config):
    ...
    # Create MyHaveibeenpwnedEntity entity with value set to 'Hello <request.value>!'
    e = MyHaveibeenpwnedEntity('Hello %s!' % request.value)
